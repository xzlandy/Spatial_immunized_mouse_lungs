...
puck <- readRDS('spatial_a4/STRef.rds')
barcodes <- colnames(puck@counts) #pixels to be used (a list of barcode names). 

reference = readRDS('transferred_rna_reference/SCRef.rds')

myRCTD <- create.RCTD(puck, reference, max_cores = 24, test_mode = F, CELL_MIN_INSTANCE = 6)
myRCTD <- run.RCTD(myRCTD, doublet_mode = 'full')

results <- myRCTD@results
save(myRCTD, file = 'spatial_a4_RCTD_transferred_rna_reference.RData')
